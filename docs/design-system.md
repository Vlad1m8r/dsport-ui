...
- По умолчанию выделение текста отключено для обычных элементов интерфейса (включая Telegram WebView).
- Для `input`, `textarea` и `[contenteditable="true"]` выделение и курсор всегда включены.
- Для точечных случаев копирования (ID/ссылки/коды) используем атрибут `data-selectable="true"`.
- Причина: предотвращение случайного выделения текста и вызова копи-меню при long-press.
